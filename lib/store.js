...
// Partial Payment — customer ne kitna diya
// Udhaar se amount minus hoga, payment history me save hoga
export async function addPayment(uid, customerId, amount, method = "cash") {
  // Payment record banao
  await addDoc(collection(db, "shops", uid, "payments"), {
    customerId,
    amount: Number(amount),
    method,
    createdAt: serverTimestamp(),
  });
  // Customer ka udhaar update karo
  const customerRef = doc(db, "shops", uid, "customers", customerId);
  await updateDoc(customerRef, {
    udhaar: increment(-Number(amount)),
  });
}

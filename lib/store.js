...
// Cancel/Delete order — udhaar bhi wapas hoga
export async function cancelOrder(uid, orderId, order) {
  await deleteDoc(doc(db, "shops", uid, "orders", orderId));
  if (order.customerId) {
    const customerRef = doc(db, "shops", uid, "customers", order.customerId);
    const udhaarToRemove = order.paid ? 0 : Number(order.total || 0);
    await updateDoc(customerRef, {
      totalOrders: increment(-1),
      udhaar: increment(-udhaarToRemove),
    });
  }
}

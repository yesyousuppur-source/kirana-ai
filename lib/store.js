...
export async function addOrder(uid, order) {
  ...
  if (order.customerId) {
    ...
    const paidIncrement = order.paid ? Number(order.total || 0) : 0;
    await updateDoc(customerRef, {
      totalOrders: increment(1),
      udhaar: increment(udhaarIncrement),
      totalPaid: increment(paidIncrement),
...
export async function markOrderPaid(uid, orderId, order, paymentMethod = "cash") {
  ...
  if (order.customerId && order.total) {
    ...
    await updateDoc(customerRef, {
      ...
      totalPaid: increment(Number(order.total)),
    });
  }
}
...
// Partial Payment — udhaar kam + totalPaid update
export async function addPayment(uid, customerId, amount) {
